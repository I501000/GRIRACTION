...
function proxyStrategy(destination) {
    ...
    if (!getProxyEnvValue(destinationProtocol)) {
        logger.debug(`No Proxy settings for ${destinationProtocol} are found in environment variables - no proxy used`);
...
function getProxyEnvValue(protocol) {
    ...
    logger.debug(`Try to fetch ${proxyEnvKey.toLowerCase()} or ${proxyEnvKey.toUpperCase()} from the process env. Found value is ${proxyEnvValue}`);
    if (!proxyEnvValue) {
        return undefined;
    }
