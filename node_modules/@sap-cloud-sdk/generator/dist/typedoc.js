...
function getNavPropertyDescription(property) {
    return `${property.isCollection ? 'One-to-many' : 'One-to-one'} navigation property to the [[${property.toEntityClassName}]] entity.`.trim();
}
...
function getComplexTypePropertyDescription(property, complexTypeName) {
    return `Representation of the [[${complexTypeName}.${property.instancePropertyName}]] property for query construction.${util_1.unixEOL}Use to reference this property in query operations such as 'filter' in the fluent request API.`;
}
...
function getStaticPropertyDescription(property) {
    return `Static representation of the [[${property.instancePropertyName}]] property for query construction.${util_1.unixEOL}Use to reference this property in query operations such as 'select' in the fluent request API.`;
}
...
function getStaticNavPropertyDescription(property) {
    return `Static representation of the ${property.isCollection ? 'one-to-many' : 'one-to-one'} navigation property [[${property.instancePropertyName}]] for query construction.${util_1.unixEOL}Use to reference this property in query operations such as 'select' in the fluent request API.`;
}
